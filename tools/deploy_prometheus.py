...
def deploy_prometheus_sub(olm_ns, cat_src):
    ...
    with open(src_file, "r") as src:
        with open(dst_file, "w+") as dst:
            ...
            dst.write(data)
    utils.apply(dst_file)
    utils.wait_for_rollout('deployment', 'prometheus-operator', args.namespace)
...
def main():
    if args.target != "oc-ingress":
        ...
        sleep(10)
        utils.check_k8s_rollout('statefulset', 'prometheus-assisted-installer-prometheus', args.namespace)
